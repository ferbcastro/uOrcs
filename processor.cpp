...
// =============================================================================
processor_t::processor_t() : cycle(0), 
                             total_branches(0), 
...
void processor_t::allocate() {
    btb = new btb_t;
    predictor = new combined_predictor_t;
}
...
void processor_t::clock() {
    ...
    if (btb->was_branch_cond == true) {
        ...
        if (has_jumped) {
            predictor->increment_predictor(GSHARE, op_address);
            predictor->increment_predictor(BIMODAL, op_address);
        }
        else {
            predictor->decrement_predictor(GSHARE, op_address);
            predictor->decrement_predictor(BIMODAL, op_address);
        }
    ...
    if (!orcs_engine.simulator_alive) return;
...
void processor_t::statistics() {
    ...
    ORCS_PRINTF("error rate:%ld%%\n", num_wrong_predictions * 100 / total_branches);
    ORCS_PRINTF("number of btb misses:%lu\n", num_misses);
